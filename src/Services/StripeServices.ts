...
export default class StripeServices {
  ...
  public structurePricingData(data:any){
    ...
    const PricingPlansCardData = [
      ...
      if(plan_type=='basic'){
  const product=PricingPlansCardData.filter((itm)=>itm.key==prod?.prod_meta?.name)[0]
    if(product){
      return {...product,planPrice:prod?.price?.amount,planName:prod?.prod_name,...prod}
       }
      return null 
      }else if(plan_type=='premium'){
        const product=PricingPremiumPlansCardData.filter((itm)=>itm.key==prod?.prod_meta?.name)[0]
          if(product){
            return {...product,planPrice:prod?.price?.amount,planName:prod?.prod_name,...prod}
          }
          return null 
